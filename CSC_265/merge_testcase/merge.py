...
args = sys.argv
if len(args) < 3:
	print("Usage: python3 merge.py <key column name> <file 1> <file 2> <file 3> ...", file=sys.stderr)
	exit()
...
for file_name in file_list:
	try:	
		...
		if info.st_size == 0:
			eprint(EMPTY_TABLE_ERROR)
			sys.exit()	
		
		f = open(file_name)
	except FileNotFoundError:
		eprint("Error: Unable to open %s"%(file_name))
		sys.exit()
	line = ""	
	while "," not in line:
...
if key_word not in headings:
	eprint(KEY_COLUMN_MISSING_ERROR)
	sys.exit()
...
for i in range(len(headings)):
	if headings.count(headings[i]) > 1:
		eprint('Error: Column name "%s" occurs too many times'%headings[i])
		sys.exit()
for file_name in file_list:
	key_index = -1
	try:	
	...
	for line in f:
		...
		if is_heading == 1:
			if prev_num != num_values:
				eprint(INCONSISTENT_ROWS_ERROR)
				sys.exit()
		...
		if is_heading == 0:
			for i in range(len(row)):
				if row[i] == key_word:
					key_index = i
			if key_index == -1:
				eprint('Error: File %s No column called "%s"'%(file_name, key_word))
				sys.exit()
			del row[key_index]
			...
			if row[key_index] in table_dict:
				eprint("Error: The row key %s occurs multiple times in the table %s"%(row[key_index], file_name))
				sys.exit()
			else:
